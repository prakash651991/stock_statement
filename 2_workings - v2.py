...
for i in range(1):
    ...
    #print(cleaned_product)
    cbs_single_lendor_data['status'] = np.where(~cbs_single_lendor_data['Product'].isin(cleaned_product), 0, cbs_single_lendor_data['status'])
    ...
    if cbs_single_lendor_selected_pos_Value > Required_Encumbrance:
        ...
        for product in Product_l:
            ...
            cleaned_product.append(prodstring[1])
        print(cleaned_product)
        fresh_accounts['status'] = np.where(~fresh_accounts['Product'].isin(cleaned_product), 0, fresh_accounts['status'])
        ...
        #
        cnx = create_engine('mysql+pymysql://analytics:%s@34.93.197.76:61306/analytics' % quote('Secure@123'))
        Selected_additional_Accounts.to_sql(con=cnx, name='stock_report_selected_acc_set2', if_exists='append',index=False)
        cnx.dispose()
